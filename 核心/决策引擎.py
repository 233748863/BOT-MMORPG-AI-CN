    ...
    def _检查紧急规则(self, 上下文: 决策上下文) -> Optional[决策结果]:
        ...
        if 上下文.血量百分比 < 低血量阈值:
            if self.检查冷却(紧急动作):
                ...
                # 记录紧急规则触发日志 - 需求: 8.4
                logger.warning(f"⚠️ 紧急规则触发: 低血量 ({上下文.血量百分比:.1%}) < 阈值 ({低血量阈值:.1%})，执行动作: {动作名称}")
                return 决策结果(
                    动作索引=紧急动作,
                    动作名称=动作名称,
                    来源="rule",
                ...
                logger.debug(f"紧急规则触发但动作在冷却中: 低血量 ({上下文.血量百分比:.1%})")
        ...
        if 上下文.附近敌人数量 >= 被围攻阈值:
            if self.检查冷却(紧急动作):
                动作名称 = self._获取动作名称(紧急动作)
                # 记录紧急规则触发日志 - 需求: 8.4
                logger.warning(f"⚠️ 紧急规则触发: 被围攻 ({上下文.附近敌人数量}个敌人) >= 阈值 ({被围攻阈值})，执行动作: {动作名称}")
                return 决策结果(
                    动作索引=紧急动作,
                    动作名称=动作名称,
                    来源="rule",
                ...
                logger.debug(f"紧急规则触发但动作在冷却中: 被围攻 ({上下文.附近敌人数量}个敌人)")
