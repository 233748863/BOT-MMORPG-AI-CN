...
from 核心.动作空间 import 标准化动作权重, 标准化动作向量
...
class 增强版游戏AI机器人:
    ...
    def __init__(self, 模式: str = '主线任务', 启用增强: bool = True):
        ...
        # 设置动作权重
        if 模式 == '主线任务':
            self.动作权重 = np.array(标准化动作权重(训练模式.主线任务['动作权重']))
        else:
            self.动作权重 = np.array(标准化动作权重(训练模式.自动战斗['动作权重']))
    ...
    def 增强决策(self, 屏幕: np.ndarray, 模型预测: List[float]) -> tuple:
        ...
        # 降级到基础模型
        动作索引 = np.argmax(np.abs(np.array(标准化动作向量(模型预测)) * self.动作权重))
