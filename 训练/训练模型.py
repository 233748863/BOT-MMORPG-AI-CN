...
from 配置.设置 import (
    ...
    # 训练可视化配置
    启用训练可视化, 启用实时图表, 启用终端输出, 启用健康监控,
    图表更新间隔, 健康检查间隔, 自动保存训练日志, 训练日志目录, 训练静默模式
)
...
def 显示训练菜单():
    ...
    print(f"  - 训练可视化: {'启用' if 启用训练可视化 else '禁用'}")
    if 启用训练可视化:
        print(f"    - 实时图表: {'启用' if 启用实时图表 else '禁用'}")
        print(f"    - 终端输出: {'启用' if 启用终端输出 else '禁用'}")
        print(f"    - 健康监控: {'启用' if 启用健康监控 else '禁用'}")
        print(f"    - 更新间隔: 每 {图表更新间隔} 批次")
    print()
    print("可用功能模块:")
...
def 主程序():
    ...
    # 初始化训练监控器（需求 2.4: 按可配置的间隔更新图表）
    可视化 = None
    if 训练监控可用 and 启用训练可视化:
        # 使用配置文件中的默认设置
        默认启用 = 启用训练可视化
        提示文本 = f"是否启用训练可视化监控? (y/n, 默认{'y' if 默认启用 else 'n'}): "
        监控选择 = input(提示文本).strip().lower()
        
        if (默认启用 and 监控选择 != 'n') or (not 默认启用 and 监控选择 == 'y'):
            # 使用配置文件中的可视化设置创建回调
            可视化配置 = {
                "更新间隔": 图表更新间隔,
                "启用图表": 启用实时图表,
