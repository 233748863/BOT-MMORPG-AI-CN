...
- [x] 1. 实现帧比较器
  - [x] 1.1 创建 `核心/帧比较.py` 文件，实现 `帧比较器` 类
    - 实现直方图比较方法
    - 实现 SSIM 比较方法（可选）
    - _需求: 1.1, 1.2_
  - [x] 1.2 实现区域比较功能
...
- [x] 2. 实现缓存策略
  - [x] 2.1 创建 `核心/缓存策略.py` 文件，实现 `缓存策略` 类
    - 实现全局相似度阈值判断
    - 实现优先区域阈值判断
...
- [x] 3. 实现智能缓存
  - [x] 3.1 创建 `核心/智能缓存.py` 文件，实现 `智能缓存` 类
    - 封装检测器
    - 实现缓存存储和查询
    - _需求: 2.1, 2.2_
  - [x] 3.2 实现缓存过期机制
